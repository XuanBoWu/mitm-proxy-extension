...
Agent / MCP Client
        ...
        | stdio MCP
        v
mcp/secmp-mcp-server.js
        |
        | 127.0.0.1 HTTP + token
        v
SecMP MCP bridge inside extension.js
        |
...
MCP server 不读取 Webview DOM，也不直接连接 mitmweb。它通过扩展内的 MCP bridge 读取 SecMP 已规范化的 flow、body 状态和 session 缓存。
...
`secmp.mcp.port` 默认使用 `0`，由操作系统自动选择空闲端口。Agent 不需要手动填写实际端口；SecMP 会把最终 bridge URL 和 token 写入状态文件。
...
```text
~/.secmp/mcp-bridge.json
```

文件内容包含本机 bridge URL、随机 token、端口和更新时间。文件权限按 `0600` 写入。

## Agent MCP 配置

推荐在 SecMP 环境弹窗点击「复制 MCP 配置」，或在命令面板运行 `SecMP: 复制 MCP Client 配置`。该操作会自动启用 MCP bridge，并把适合当前平台的 stdio MCP 配置复制到剪贴板：

- Windows：自动使用已安装扩展目录下的 `mcp\secmp-mcp-server.js`
- macOS / Linux：自动使用已安装扩展目录下的 `mcp/secmp-mcp-server.js`
- 如果配置了 `secmp.mcp.stateFile`，复制结果会自动追加 `--state-file`
...
```json
{
  "mcpServers": {
    "secmp": {
      ...
      "command": "node",
      "args": [
        "<当前平台的 SecMP 扩展安装目录>/mcp/secmp-mcp-server.js"
      ]
    }
  }
}
...
手动配置时，也可以直接引用扩展内的 MCP server。如果使用自定义状态文件：
...
```json
{
  "mcpServers": {
    "secmp": {
      ...
      "command": "node",
      "args": [
        "/Users/alexwu/Projects/mitm-proxy-extension/mcp/secmp-mcp-server.js",
        "--state-file",
        "/path/to/mcp-bridge.json"
      ]
    }
  }
}
...
```text
SECMP_MCP_STATE_FILE=/path/to/mcp-bridge.json
SECMP_MCP_BRIDGE_URL=http://127.0.0.1:<state-file-port>
SECMP_MCP_TOKEN=...
```
...
- 检查 `~/.secmp/mcp-bridge.json` 是否存在，且内容中的 `"running"` 为 `true`
- 检查状态文件中的 `url` 是否为 `http://127.0.0.1:<port>`，不要手动依赖固定端口
