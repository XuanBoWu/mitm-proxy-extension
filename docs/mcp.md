...
手动配置时，推荐引用用户目录下的稳定 MCP server 副本。如果使用自定义状态文件：
...
- 检查 `secmp_status`、`secmp_list_flows` 或 `secmp_stats` 返回中是否包含 `staleProxyDetected: true`。如果存在，说明本机有上一个 Extension Host 遗留的 SecMP proxy，当前 MCP bridge 读不到那个进程内的抓包状态；请在当前 SecMP 窗口停止并重新启动代理
