...
- 支持导出 HAR 或 JSON。
- 提供 SecMP Activity Bar 图标和侧边栏快捷操作，可直接打开抓包面板、控制代理、设置设备代理和推送证书。
...
- SecMP VSIX 安装包。
- 首次启动代理时可访问互联网，或准备同一个 GitHub Release 中提供的 SecMP runtime zip 用于离线安装。
...
3. 选择下载好的 VSIX。
4. 点击 Activity Bar 中的 SecMP 图标并选择“启动代理”，或运行 `SecMP: Start Proxy`。
5. SecMP 会从对应 GitHub Release 自动下载匹配的 runtime，并缓存到 VS Code 全局存储目录。
...
离线安装时，可以从同一个 release 下载匹配的 `secmp-runtime-<platform>-<arch>-<version>.zip`，配置 `secmp.runtimeArchivePath`，或在提示时选择该 zip。

## 快速开始

1. 连接已开启 USB 调试的 Android 设备。
2. 点击 Activity Bar 中的 SecMP 图标并选择“打开抓包面板”，或运行 `SecMP: Show Capture Panel`。
3. 在设备面板点击刷新，确认设备已显示。
4. 启动代理，默认端口通常为 `8080`。
...
除命令面板外，Activity Bar 中的 SecMP 图标也提供常用快捷操作。
...
runtime 版本与 VSIX 版本相互独立。只要 `secmp.runtimeVersion` 不变，SecMP 会复用已缓存的 runtime；该设置变化时才会安装新的 runtime。

更新检查只判断扩展 VSIX 版本。runtime 升级仍由 `secmp.runtimeVersion` 控制，所以当 runtime API 和版本都不需要变化时，升级扩展可以继续复用已有 runtime。

可以使用 `SecMP: Clean Runtime Cache` 清理当前平台的旧 runtime 缓存。该命令默认保留当前 runtime 版本和最新的上一个版本，删除更旧的 runtime 目录和过期下载 zip，不会删除 mitmproxy CA/config 目录。
...
旧的 `secmp.windowsRuntime*` 设置仍会作为兼容别名生效，但新安装建议使用 `secmp.runtime*`。
